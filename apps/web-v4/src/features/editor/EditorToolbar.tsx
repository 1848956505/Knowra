  ...
  return (
    <div ref={props.toolbarRef} className={styles.toolbar} data-pdf-exclude="true" data-pinned={props.pinned || undefined} role="toolbar" aria-label="笔记格式工具栏">
      <div className={styles.toolbarMenus} aria-label="编辑器菜单">
        ...
        <ToolbarMenu label="段落" disabled={!props.canWrite}>
          {renderEditorParagraphMenu({ onCommand: props.onRunCommand })}
        </ToolbarMenu>
        <ToolbarMenu label="编辑">
          {renderEditorEditMenu({ canWrite: props.canWrite, onAction: props.onEditAction })}
        </ToolbarMenu>
        <ToolbarMenu label="格式" disabled={!props.canWrite}>
          <MenuItem id="bold" kbd="⌘B" onAction={command('bold')}>加粗</MenuItem>
          <MenuItem id="italic" kbd="⌘I" onAction={command('italic')}>斜体</MenuItem>
          <MenuItem id="inline-code" onAction={command('inline-code')}>行内代码</MenuItem>
        </ToolbarMenu>
        <ToolbarMenu label="视图"><MenuItem id="inspector" onAction={props.onToggleInspector}>{props.inspectorOpen ? '隐藏' : '显示'}文档检查器</MenuItem></ToolbarMenu>
      </div>
      <span className={styles.separator} aria-hidden="true" />
      <FormatButton label="一级标题" disabled={!props.canWrite} onPress={command('heading-1')}><strong>H1</strong></FormatButton>
      <FormatButton label="二级标题" disabled={!props.canWrite} onPress={command('heading-2')}><strong>H2</strong></FormatButton>
      <FormatButton label="三级标题" disabled={!props.canWrite} onPress={command('heading-3')}><strong>H3</strong></FormatButton>
      <span className={styles.separator} aria-hidden="true" />
      <FormatButton label="加粗" disabled={!props.canWrite} onPress={command('bold')}><strong>B</strong></FormatButton>
      <FormatButton label="斜体" disabled={!props.canWrite} onPress={command('italic')}><em>I</em></FormatButton>
      <FormatButton label="行内代码" disabled={!props.canWrite} onPress={command('inline-code')}><CodeIcon size={16} /></FormatButton>
      <span className={styles.separator} aria-hidden="true" />
      <FormatButton label="无序列表" disabled={!props.canWrite} onPress={command('bullet-list')}><ListIcon size={16} /></FormatButton>
      <FormatButton label="引用" disabled={!props.canWrite} onPress={command('blockquote')}><QuoteIcon size={16} /></FormatButton>
      <FormatButton label="插入表格" optional disabled={!props.canWrite} onPress={command('table')}><TableIcon size={16} /></FormatButton>
      ...
      <MenuTrigger>
        ...
        <MenuPopover placement="bottom end"><Menu ariaLabel="插入菜单"><MenuItem id="table" onAction={command('table')}>插入 3×3 表格</MenuItem><MenuItem id="image" isDisabled>插入图片（待附件适配）</MenuItem></Menu></MenuPopover>
      </MenuTrigger>
