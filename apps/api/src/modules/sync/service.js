...
export function createSyncService(provider, ownerId) {
  const describe = journal => ({ protocolVersion: 1, entitySchemaVersion: 5, datasetEpoch: journal.epoch, ownerId, scope: 'notes', capabilities: ['atomic-entities-v2', 'attachment-transfer-v1', KNOWLEDGE_SYNC_CAPABILITY, 'asset-lifecycle-v1'], pushEnabled: process.env.KNOWRA_SYNC_PUSH_ENABLED !== 'false', cursor: cursorFor(journal, ownerId) });
  return {
