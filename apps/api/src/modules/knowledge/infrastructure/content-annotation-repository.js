export function createInMemoryContentAnnotationRepository(options = {}) {
  ...
  const active = (item, includeDeleted) => includeDeleted || (item.lifecycleStatus ?? item.status) === 'active';
  return {
    ...
    findDuplicate({ noteId, kind, scopeType, anchorFingerprint, quoteText, fromPosition, toPosition }) { return records.map(normalized).find((item) => item.noteId === noteId && item.lifecycleStatus === 'active' && item.kind === kind && (anchorFingerprint ? item.anchorFingerprint === anchorFingerprint && item.scopeType === scopeType : item.quoteText === quoteText && item.fromPosition === fromPosition && item.toPosition === toPosition)) ?? null; },
