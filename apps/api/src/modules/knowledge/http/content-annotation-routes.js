...
export async function handleContentAnnotationRoute({ request, response, url, knowledge }) {
  ...
  if (request.method === 'DELETE' && !action) { sendJson(response, 200, { data: await knowledge.deleteAnnotation({ id }) }); return true; }
  if (request.method === 'POST' && action === 'restore') { sendJson(response, 200, { data: await knowledge.restoreAnnotation({ id }) }); return true; }
