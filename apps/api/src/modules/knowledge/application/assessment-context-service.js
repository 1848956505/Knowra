...
export function createAssessmentContextService({
  ...
  function assertIdAvailable(repository, id, code, entityName, collection) {
    if (getTombstone?.(collection, id)) throw conflictError('TRAINING_ASSET_ID_DELETED', '已清理的考试资产 ID 不能复用');
  ...
  const profileService = {
    ...
    list(options = {}) { return examProfileRepository.list(options); },
  ...
  const focusService = {
    create(input = {}) {
      ...
      const duplicate = examFocusRepository.list({ examProfileId: dto.examProfileId, learningObjectiveId: dto.learningObjectiveId, includeArchived: true, includeDeleted: true })[0];
