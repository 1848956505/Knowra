...
> 2026-09-21（2.24.0）。面向 Apple Silicon Mac 的个人使用版本。交付 `.app`，不走 App Store，不配置 Developer ID、公证或自动更新。本轮通过隔离资料库进行自动化验收，无需用户远程手动测试。
...
知识库中可创建、编辑、确认、归档和恢复候选。表单输入会保留恢复草稿，异常退出后可在知识库恢复；恢复沿用原来源和编辑版本，发生并发变化时会拒绝覆盖。同步冲突处理前自动保存恢复记录。
...
- 底部同步面板的“备份与恢复”提供备份列表、检查和恢复。检查包含文件哈希、SQLite 完整性、资料引用及附件内容；检查通过后，勾选确认再恢复整个资料库。
...
窗口启用 context isolation、sandbox 并禁用 Node integration；预加载只暴露保存握手和受限恢复草稿读写。外部链接交给系统浏览器，本地服务保持回环会话隔离。
