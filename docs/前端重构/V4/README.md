...
- 已完成：`V4-07 资料编辑器`（文件、段落、编辑、格式、视图、正文右键快捷面板与文档检查器；信息/大纲/链接 Tab、阅读/编辑/专注、双侧区域、浅色源码分栏、内部链接、查找替换、Markdown 导入、真实 PDF、顺序自动保存、标签工作区以及附件列表/上传/重命名/删除/正文插入均已接入）
- 后续能力：`AI 检查`仍需以明确产品契约接入；不影响当前附件能力可用性。
...
| 雾青 / 方册 / 澄明历史候选 | 均未采用；原候选文档未随当前仓库保留，不再提供失效入口 |
