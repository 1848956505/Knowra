...
日期：2026-09-26。状态：**代码与离线验收完成；真实账号能力探测待预算执行器和账号验收。** 不把 Mock 或公开文档当作当前账号已通过。
...
- JSON/PostgreSQL 的应用上下文装配[运行时](../../../apps/api/src/modules/ai/runtime.js)；Mac 本地服务也装配 Gateway。当前没有生成 HTTP/IPC 路由。真实 adapter 在预算授权回调未注入时**默认拒绝调用**，且拒绝发生在读取密钥之前。AI-01-04 的权威预算预留完成后才注入该回调。
...
| 真实 DeepSeek 生成 | 未运行。10 元/日、2 元/任务的跨端权威预算执行器尚未实现。 |
