        ...
        {filtered.length === 0 ? (
          <section className="entry-list">
            <div className="empty-state">
              <MagnifyingGlass className="empty-state-icon" weight="duotone" aria-hidden="true" />
                ...
                >
                  ...
                  <div className="entry-copy">
                    <div className="entry-heading">
                      ...
                      <span className={`status status-${note.status}`}><i></i>{getStatusLabel(note.status)}</span>
                    </div>
                    <p>{note.summary}</p>
          ...
          <>
            ...
            <section className="inspector-fixed-section">
              ...
              <dl className="inspector-record">
                ...
                <div><dt>状态</dt><dd><span className={`status status-${currentNote.status}`}><i></i>{getStatusLabel(currentNote.status)}</span></dd></div>
                  ...
                  <div className="editor-menu-popover-host" key={key}>
                    ...
                    {openMenu === key && (
                      <div className="editor-menu-popover" role="menu">
                        {items.map((it) => {
                          const sc = MENU_SHORTCUTS[it];
                          return (
                            <button type="button" key={it} className="editor-menu-item">
                              <span>{it}</span>
                              {sc && <kbd className="editor-menu-shortcut">{sc}</kbd>}
                            </button>
                          );
                        })}
                      </div>
              ...
              <div className="document-meta-row">
                <div className="document-location">
                  <span className="breadcrumb">
                    <span className="breadcrumb-item">资料库</span>
                    <span className="breadcrumb-item">{FOLDERS.find((f) => f.id === note.folderId)?.name || '未分类'}</span>
                    <span className="breadcrumb-item breadcrumb-current">{note.id.slice(-3).toUpperCase()}</span>
                  </span>
                  <span className={`status status-${note.status}`}><i></i>{getStatusLabel(note.status)}</span>
                </div>
