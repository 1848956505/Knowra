...
`scripts/post-deploy.sh` 会构建 V4、拒绝携带 Source Map 的生产产物，并通过 `deploy/ecosystem.config.cjs` 将 PM2 的 `knowra-web` 入口刷新为 `apps/web-v4/server.mjs`。Nginx 仍反向代理本机 `3000`，无需改变公网路由。V3 仅保留为回归比较与紧急回退入口。
